...
class TestAndroidDashboard:
    def test_dashboard_empty(self, client):
        response = client.get("/api/v1/android/dashboard")
    ...
    def test_dashboard_with_servers(self, client, db_session):
...
class TestAndroidServerDetails:
    def test_server_details(self, client, db_session):
        from models.server import Server
        from models.metric import Metric
        ...
        response = client.get(f"/api/v1/android/servers/{server.id}/details")
        assert response.status_code == status.HTTP_200_OK
        data = response.json()
    ...
    def test_server_details_not_found(self, client):
        response = client.get("/api/v1/android/servers/999/details")
        assert response.status_code == status.HTTP_404_NOT_FOUND


class TestAndroidMetricsHistory:
    def test_metrics_history(self, client, db_session):
        from models.server import Server
        from models.metric import Metric
        ...
        response = client.get(f"/api/v1/android/servers/{server.id}/metrics/history")
        assert response.status_code == status.HTTP_200_OK
        data = response.json()
